...
bool PresetMigration::migrateToCurrent(juce::ValueTree &state) {
  if (!state.isValid() || state.getType().toString() !=
  ...
  int version =
      state.getProperty(juce::String(std::string(schema::versionAttr)), 0);
  ...
  if (version < 1) {
    ...
    state.setProperty(juce::String(std::string(schema::versionAttr)), version,
                      nullptr);
  }
    ...
    if (child.getType().toString() == "PARAM") {
      ...
      if (idString == "macro.roller_dyn") {
        child.setProperty("id",
                          juce::String(std::string(keys::macroRollerDynamics)),
                          nullptr);
      }

      // Cleanup dead nodes so they don't pollute the new tree
      if (idString == "sub.shape" || idString == "sub.phase" ||
          idString == "sys.xover" || idString == "osc.fm.ratio" ||
          idString == "fx.ott.mix") {
        // Ignored by APVTS
      }
    }
